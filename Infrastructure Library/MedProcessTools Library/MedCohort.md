# MedCohort
### [MedCohort ](https://Medial-EarlySign.github.io/medpython/classMedCohort)is a data structure with helpers to deal with a cohort, a list of individuals with (dated) outcomes and followup times.
MedCohort contatins a vector of basic records ([CohortRec](https://Medial-EarlySign.github.io/medpython/structCohortRec)), each representing a single period for a specific id (with a corresponding outcome) information.
A MedCohort can be sampled to generate MedSamples files according to [SamplingParams](https://Medial-EarlySign.github.io/medpython/structSamplingParams) using one of two fuctions:
...
A MedCohort can also be used to estimate the age and gender dependent incidence rate. Estimation is done using the following function which according to [IncidenceParams](https://Medial-EarlySign.github.io/medpython/structIncidenceParams):
